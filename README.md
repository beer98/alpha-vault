...
├── TEMPLATE.html       ← 新研究页「外壳」模板（红黑 house style；只锁字体/配色/返回/免责，正文自由）
├── assets/             ← 以后放共享图片
└── r/                  ← 每一份研究各占一个文件夹
...
- **研究内页**：`TEMPLATE.html` 是红黑外壳（与目录页同系：Fraunces + IBM Plex Mono + 纸/墨/朱红），只锁字体、配色、返回导航、底部免责声明；**正文每篇结构自由**，要积木就从纳德拉页抄 `.step`/`.lyr`/`.snap`/`.seal`/`.tk`。每页底固定：基于公开信息、非投资建议、标的自验。
